...
export default function PriorityView({ telemetry, lastUpdate, darkMode }) { 
  ...
  const avgSpeed = telemetry?.motion?.avgSpeed ?? 0; 
  const posX = telemetry?.motion?.lx ?? 0; 
  const reflectorCount = telemetry?.navigation?.reflectorCount ?? 0;
      ...
      <Box sx={{ display: "grid", gridTemplateColumns: "1fr 320px", flexGrow: 1, gap: 2, minHeight: 0 }}> 
        ...
        <Box sx={{ display: "flex", flexDirection: "column" }}> 
          <Box sx={{ 
            ...
            {[ 
              { label: "HIZ (m/s)", vals: [speedX, telemetry?.motion?.sy ?? 0, telemetry?.motion?.sz ?? 0] }, 
        ...
        <Box sx={{ display: "flex", flexDirection: "column", height: "100%" }}> 
          ...
          <Box sx={{ display: "flex", flexDirection: "column", gap: 1.2, px: 1 }}> 
            ...
            <Box sx={{ mt: 1 }}> 
              ...
              <Box sx={{ display: "flex", gap: 0.5 }}> 
                <SmallDataBox label="B1" value={telemetry?.temperature?.ct1 ?? "--"} /> 
                <SmallDataBox label="B2" value={telemetry?.temperature?.ct2 ?? "--"} /> 
                <SmallDataBox label="B3" value={telemetry?.temperature?.ct3 ?? "--"} /> 
              </Box> 
            </Box> 
          ...
          <Box sx={{ px: 1, mt: 2 }}> 
              <Box sx={{ background: "rgba(15, 23, 42, 0.6)", border: "1px solid rgba(125, 207, 255, 0.2)", borderRadius: 1, py: 1, display: "flex", flexDirection: "column", alignItems: "center" }}> 
                  <Typography sx={{ fontSize: 8, color: "#7dcfff", fontWeight: "bold" }}>REFLEKTÖR SAYACI</Typography> 
                  <Typography sx={{ fontSize: 18, color: "#fff", fontWeight: "bold", fontFamily: "monospace" }}>{reflectorCount}</Typography> 
                </Box> 
          </Box> 
          ...
          <Box sx={{ display: "flex", gap: 1.2, px: 1, pb: 9.5, height: 125, flexShrink: 0 }}> 
            <Box sx={{ flex: 1, background: "rgba(30, 41, 59, 0.3)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 2 }}> 
              <NeonGauge value={speedX} max={300} label="ANLIK HIZ" color="#00e676" /> 
            </Box> 
            <Box sx={{ flex: 1, background: "rgba(30, 41, 59, 0.3)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 2 }}> 
